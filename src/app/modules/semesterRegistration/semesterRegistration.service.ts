...
const insertIntoDB =async (data: SemesterRegistration):Promise<SemesterRegistration> => {
    ...
    const isAnySemesterRegistrationUpcomingOrOngoing = await prisma.semesterRegistration.findFirst({
        where:{
            OR:[
                {
    ...
    if(isAnySemesterRegistrationUpcomingOrOngoing){
        throw new ApiError(httpStatus.BAD_REQUEST,`There is ALready Have The ${isAnySemesterRegistrationUpcomingOrOngoing.status} Registration`)
    }
